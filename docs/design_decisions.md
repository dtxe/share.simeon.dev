...
- Reasoning is hard-capped via Fireworks' Anthropic-compatible `thinking: {"type":"enabled","budget_tokens": ...}` (set to half of `extractionMaxTokens`, i.e. 2000 of 4000). This guarantees the model's thinking phase can't exhaust `max_tokens` and leave the forced `extract_receipt` tool call truncated with `finish_reason:"length"` — the case that would break the `DisallowUnknownFields` decode of the tool-call JSON. Both supported Fireworks models (MiniMax M3 and Kimi K2.7 Code) accept this form (live-verified); the integer form of `reasoning_effort` documented elsewhere in Fireworks' spec is rejected by both with 400 "expected string," and the qualitative string `reasoning_effort:"low"` is not a hard cap, so neither is used. Unknown models (e.g. via `LLM_PROVIDER=openai` against a non-Fireworks endpoint) get no `thinking` field — which would 400 on endpoints that don't understand it — and instead keep the prompt-level `minimizeReasoningPromptSuffix` asking the model to call the extraction tool directly. A `finish_reason:"length"` warn-log was added so a future receipt large enough to overflow even the remaining 2000 tool-call tokens is observable.
