...
- **`createImageBitmap` exists on iOS Safari but can't decode HEIC** — it throws, instead of falling back. A naive feature-gate on `'createImageBitmap' in window` then makes the `<img>.decode()` path (which is the *only* one that reaches iOS's OS-level native HEIC decoder) unreachable, so HEIC→canvas conversion silently returns null on iPhone and receipt upload fails with a generic "could not read this photo" message. Always wrap `createImageBitmap` in `try/catch` and fall through to `<img>.decode()`. (Hit in `frontend/src/lib/image.ts`; the intent of the canvas-first path only worked once this fallback was wired in.) Bonus companion trap: HEIC detection by `file.type`/extension alone is unreliable — iOS often hands back `type=''` for camera/library HEIC. Sniff the `ftyp` box magic bytes (brands `heic`/`heix`/`mif1`/etc.) as well.
- **`<input type="file" accept="image/*" capture="environment">` forces the camera on iOS and hides the photo-library picker.** That's the HTML spec's defined behavior for `capture`, not a bug — but it makes a button labeled "photo or upload" unable to do the upload half on mobile. Use `capture` only for a camera-only affordance; drop it whenever the user should be able to pick an existing photo. (Was on the receipt upload input in `ReceiptSection`; removed so library HEIC photos route through the converter above.)
